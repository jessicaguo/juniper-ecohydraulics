...
load("scripts/model-gpp-nirv/coda/coda-env.Rdata")
load("scripts/model-gpp-nirv/coda/codarep-env.Rdata")
...
fig5a <- ggplot() +
  ...
  geom_point(data = pos_neg,
             aes(x = Covariate, y = pred.upper + 0.25),
             pch = 8,
             col = "gray70",
             stroke = 1) +
...
fig5
ggsave(filename = "scripts/model-gpp-env/figs/fig_5.png",
       plot = fig5,
       width = 8, height = 6,
       units = "in")
...
ggsave(filename = "scripts/model-gpp-env/figs/fig_S2.png",
       plot = figS2,
       width = 5, height = 5,
       units = "in")
