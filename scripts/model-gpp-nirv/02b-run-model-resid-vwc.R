...
# Linear function of VWC10 (antecedent)
...
#### PART 2, verson with only antecedent VWC10 ####
...
init3 <- function() {
  list(B = rnorm(dat_list3$NParam, 0, 10),
       tau = runif(1, 0, 1),
       deltaB = runif(dat_list3$nlagB, 0, 1)
  )
}

inits_list3 <- list(init3(), init3(), init3())

load(file = "scripts/model-gpp-nirv/inits/saved_state-resid-vwc.Rdata")

# Initialize model
jm3 <- jags.model("scripts/model-gpp-nirv/model-resid-vwc.jags",
                 data = dat_list3,
                 inits = saved_state3[[2]],
                 n.chains = 3)

# update(jm3, n.iter = 10000)
dic.samples(jm3, n.iter = 3000, thin = 50)
...
caterplot(coda.out3, regex = "^wB\\[", reorder = FALSE)
...
# final[[1]]
# saved_state3 <- removevars(final, variables = c(2:3, 5, 7))
# saved_state3[[1]]
# 
# save(saved_state3, file = "scripts/model-gpp-nirv/inits/saved_state-resid-vwc.Rdata")
...
# Save out
save(coda.rep3, file = "scripts/model-gpp-nirv/coda/codarep-resid-vwc.Rdata")
...
sm3 <- summary(m3) # R2 = 0.1212

ggplot(pred3) +
  geom_abline(intercept = 0, slope = 1, col = "black",
              linewidth = 1) +
  geom_abline(intercept = coef(sm3)[1,1], 
              slope = coef(sm3)[2,1], 
              col = "black",
              lty = 2) +
