...
load("scripts/model-gpp-nirv/coda/coda-main.Rdata")
...
fig7b <- ggplot() +
  geom_hline(data = dummy, 
             ...
             linewidth = 0.8, 
             color = "gray70") +
  geom_errorbar(data = filter(A, Covariate != "Intercept"), 
                aes(x = Covariate,
                    ymin = pred.lower,
                    ymax = pred.upper),
                width = 0) +
  geom_point(data = filter(A, Covariate != "Intercept"), 
             aes(x = Covariate, 
                 y = pred.mean)) +
...
fig7_bot <- plot_grid(fig7b, fig7c,
                  ncol = 2,
                  labels = c("b", "c"),
                  rel_widths = c(1.65, 1),
                  label_size = 12)

fig7 <- plot_grid(fig7a, fig7_bot,
                  ncol = 1,
                  labels = c("a", ""),
                  # rel_widths = c(1.5, 1),
                  label_size = 12)
