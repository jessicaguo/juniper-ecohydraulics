...
```{r, message=FALSE}
psy <- read.csv("../data_raw/Merged-psychrometer-data.csv") %>%
...
Finally, let's check the range of psychrometer values for each tree and logger. 
...
```{r, message=FALSE, warning=FALSE}
maint <- read.csv("../data_raw/Maintenance-times.csv") %>%
...
```{r, message=FALSE, warning=FALSE}
skip <- read.csv("../data_raw/Psy-skip-dates.csv") %>%
