...
institute:
  ...
  - GDPE: Graduate Degree Program in Ecology, Colorado State University, Fort Collins, Colorado
...
-   Flexible hydraulic regulation appears to allow *J. osteosperma* to prolong soil water extraction and the period of high carbon uptake following episodic rain events during the growing season.
    Although the underlying physiological mechanisms need further study, this flexibility may underlie the life history strategy of some drought-tolerant species and suggests why dynamic vegetation models often underestimate the productivity of dryland ecosystems.
...
Along the soil-plant-atmosphere continuum, gradients of water potential ($\Psi$) drive water transport and integrate the tradeoff between obtaining carbon dioxide for photosynthesis and water loss through stomata [@berry2010].
...
Current methods to quantify drought impacts on ecosystem productivity often rely on remotely-sensed indices of vegetation greenness [@zeng2022].
However, while such indices capture structural capacity for photosynthesis on a seasonal basis, they fail to cpature the immediate constraints imposed by soil and atmospheric drought, which must be explicitly represented [@stocker2018].
The divergence between low seasonality in remote-sensed indices and high seasonality in tower-based measurements is especially apparent in semi-arid and evergreen ecosystems [@garbulsky2008, @pierrat2021].
...
We employed a Monteith-style framework [@monteith1972] to model tower-based GPP as a function of fraction of absorbed photosynthetically active radiation (fAPAR) and light use efficiency (LUE):

$$
\begin{array}{c}
GPP = PAR \cdot fAPAR \cdot LUE
\end{array}
$${#eq:1}

fAPAR is commonly represented by remotely-sensed vegetation indices; here, we used the near infrared reflectance of vegetation (NIRv) from NASA's Moderate Resolution Imaging Spectoradiometer (MODIS).
500m resolution MODIS data for US-CdM were screened for the highest quality flags and interpolated to daily values using a Savitzky-Golay filter of order \_ and length \_.
Site-level incoming photosynthetically active radiation (PAR) was obtained the tower sensor.
NIRv is well-correlated with modeled absorbed photosynthetically active radiation, even at low vegetation cover \[badgley2017\]; the product of NIRv and incoming PAR is termed NIRvP and considered a robust proxy of photosynthesis globally [@dechant2022].
...
Antecedent importance weights, vectors of length $T_{lag}$ (Eqn. 5), were given non-informative Dirichlet priors that assume *a priori* that each past time step has equal importance, and that constrain weights for each covariate to sum to 1 across all time steps, $p$.
...
$\overline{GPP}_j$ was modeled as a linear function of NIRv, PAR, and their interaction, representing the initial $GPP = fAPAR$ component of the Monteith formulation.
...
To account for the remaining effects of LUE, we calculated the residuals of the above model as the posterior mean of $GPP_j - \overline{GPP}_j$; residuals were then scaled for improved model mixing.
...
where $\overline{resid}_k$ is the predicted residual between observed and modeled (Eqn. 7, 8) GPP, and the variance $\sigma_{resid}^2$ represents the uncertainty in observed $resid$.
$\overline{resid}_k$ is interpreted as a dynamic LUE constraint on the GPP \~ fAPAR relationship.
We devised three LUE formulations that account for the combined impact of concurrent VPD and antecedent soil moisture (Eqn. 10), antecedent soil moisture alone (Eqn. 11), and concurrent predawn water potential (Eqn. 12):
...
Antecedents weights for $W_{10}^{ant}$ were constructed identically to Eqn.
5, with unique weights determined by the GPP residuals.
...
Model comparison criteria for the three forms of GPP residual models included posterior predictive loss [@gelfand1998] and $R^2$ between observed and predicted values.
...
At a coarse scale, the dynamics of hydraulic regulation can be visually estimated by plotting stem $\Psi_{MD}$ and $\Psi_{PD}$ for each season (Fig. \@ref(fig:fig-sig)).
...
The hydraulic regulation model (Eqn. 2-6) fit the data very well (Fig. S1, observed vs. predicted $\Psi_{MD}$ $R^2$= 0.920) with low bias (slope of observed vs. predicted = 0.919).
...
Although general trends in hydraulic regulation can be inferred from grouping $\Psi_{MD}$ and $\Psi_{PD}$ by season (e.g., Fig. \@ref(fig:fig-sig)), the hierarchical Bayesian model permitted combining posterior parameter distributions with environmental covariates to produce daily timeseries of predicted $\sigma$ (e.g., Fig. \@ref(fig:fig-hydry-gpp)a), which cannot be determined empirically.
...
The initial GPP model (Eqn. 7, 8) also fit the data well (Fig. \@ref(fig:fig-gpp-part1)c, observed vs. predicted $GPP$ $R^2$= 0.733), although with some degree of bias (slope of observed vs. predicted = 0.732) such that some high GPP values were underpredicted.
Comparing the GPP and NIRv time series (Fig. \@ref(fig:fig-gpp-part1)a), the first major pulse event elicited a strong GPP response prior to any green up detected optically by NIRv, partially accounting for the underprediction of some high GPP values.
...
GPP was positively associated with NIRv and the interaction between NIRv and PAR (Fig. \@ref(fig:fig-gpp-part1)b), though PAR alone was not significantly associated with GPP.
...
Three functional forms of the residual model were tested: environmental covariates with D and $W_{10}^{ant}$ (Eqn. 4, 10), soil water content with $W_{10}^{ant}$ only (Eqn. 4, 11), and predawn water potential with $\overline{\Psi_{PD}}$ only (Eqn. 12).
Of the three models, predawn water potential has the strongest coefficient of determination, lowest posterior predictive loss, and fewest effective number of parameters (Table. S1).
...
The predawn water potential model (Eqn. 12) fit the GPP residuals moderately well (Fig. \@ref(fig-gpp-resid-pd)c, observed vs. predicted $resid$ $R^2$= 0.199), and a fair amount bias was apparent.
Notably, the temporal pattern of $\overline{\Psi_{PD}}$ matched that of GPP residuals remarkably well.
$\overline{\Psi_{PD}}$ was significantly and positively correlated with the GPP residuals (Fig. \@ref(fig-gpp-resid-pd)b).
Taken together, the initial GPP model and best-performing LUE model using only predawn water potential improved the overall model fit to about $R^2$= 0.786.

Interestingly, the other models that modified LUE with environmental covariates (D, $W_{10}^{ant}$) and soil water content ($W_{10}^{ant}$ only) did not perform as well (Table S1).
In the environmental covariates model, neither D nor $W_{10}^{ant}$ was significantly correlated to the GPP residuals (Fig. S2).
This is likely because D and PAR are highly correlated (Fig. @ref(fig:fig-env)), while the residuals represent remaining variation after accounting for PAR and NIRv.
...
In this study, we set out to improve our understanding of temporal variability in plant hydraulic regulation and its relationship to carbon uptake.
We leveraged contemporaneous, high-resolution water potential and carbon flux data to compare temporal trends in daily plant water potential, GPP, and hydraulic behavior in a dryland ecosystem.
...
Temporal heterogeneity in plant hydraulic strategy and spatial heterogeneity in topoedaphic characteristics may also interact, as evidenced by high variance in stem $\Psi$ among seven co-located trees (Fig. \@ref(fig:fig-psy)a), and contribute to model underperformance in dryland ecosystems.
...
Evaluating plant responses at intermediate timescales can reveal the important contribution of hydraulic regulation to the magnitude and timing of dryland ecosystem fluxes.
