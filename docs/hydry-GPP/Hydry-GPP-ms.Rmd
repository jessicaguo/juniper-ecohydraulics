...
where $\overline{resid}_k$ is the predicted residual between observed and modeled (eqn. 6, 7) GPP, and the variance $\sigma_{resid}^2$ represents the uncertainty in observed $resid$.
$\overline{resid}_k$ is interpreted as a dynamic LUE constraint on the GPP \~ fAPAR relationship.
We devised three LUE formulations that account for the combined impact of concurrent VPD and antecedent soil moisture (eqn. 9), antecedent soil moisture alone (eqn. 10), and concurrent predawn water potential (eqn. 11):
...
Antecedents weights for $W_{10}^{ant}$ were constructed identically to Eqn 4, with unique weights determined by the GPP residuals.
...
During the monsoon period, multiple slopes are evident, with $\sigma > 1$ occurring when both W and $\Psi_{PD}$ are high; the same region during the fall season has a much shallower slope.
...
The hydraulic regulation model (Eqn. 1-5) fit the data very well (Fig. S1, observed vs. predicted $\Psi_{MD}$ $R^2$= 0.920) with low bias (slope of observed vs. predicted = 0.919).
With a much lower sample size, the GPP model (Eqn. 4, 6-7) also fit the data well (Fig. S2, observed vs. predicted $GPP$ $R^2$= 0.741), although with some degree of bias (slope of observed vs. predicted = 0.762) such that high GPP values were underpredicted.
