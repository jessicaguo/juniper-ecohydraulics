...
$$
\begin{array}{c}
\overline{resid}_k = \theta_0 + \theta_1 \cdot {\Psi_{PD}}_k
\end{array}
$${#eq:12}

where $\Psi_{PD}$ were the gapfilled means of predawn water potential at the site level.
...
```{r fig-env, echo=FALSE, out.width="100%", fig.cap="Daily site-level environmental characteristics of <b>a)</b> maximum vapor pressure deficit (D) and total precipitation and <b>b)</b> mean volumetric water content at 5 cm and 10 cm. Labeled boxes subdivide the study period into before, during, and after the monsoon season"}
...
#### Plant water potential enhances GPP modeling

The GPP model (Eqn. 5, 7-12) also fit the data well (Fig. \@ref(fig:fig-gpp-part1)c, observed vs. predicted $GPP$ $R^2$= 0.733), although with some degree of bias (slope of observed vs. predicted = 0.732) such that some high GPP values were underpredicted.
...
Conversely, low, near-zero GPP in mid to late July was not matched by similar lows in scaled PAR or NIRv, resulting in overprediction of low GPP values.\
